/**
 * wire-changelog.js: Wire data-i18n into changelog.html
 */
const fs = require('fs');
const path = require('path');
const FILE = path.resolve(__dirname, '..', 'changelog.html');
...
// Page header
rep('<h1 class="changelog-title">Changelog</h1>','<h1 class="changelog-title" data-i18n="changelogTitle">Changelog</h1>','changelogTitle');
rep('<p class="changelog-subtitle">What shipped. Real commits from our repository — not a roadmap, not a promise.</p>','<p class="changelog-subtitle" data-i18n="changelogSubtitle">What shipped. Real commits from our repository — not a roadmap, not a promise.</p>','changelogSubtitle');

// Release tags
rep('<span class="release-tag tag-fix">Accuracy</span>','<span class="release-tag tag-fix" data-i18n="changelogTagAccuracy">Accuracy</span>','changelogTagAccuracy');
rep('<span class="release-tag tag-major">Major</span>','<span class="release-tag tag-major" data-i18n="changelogTagMajor">Major</span>','changelogTagMajor');
rep('<span class="release-tag tag-feature">Feature</span>','<span class="release-tag tag-feature" data-i18n="changelogTagFeature">Feature</span>','changelogTagFeature');
rep('<span class="release-tag tag-security">Security</span>','<span class="release-tag tag-security" data-i18n="changelogTagSecurity">Security</span>','changelogTagSecurity');
rep('<span class="release-tag tag-major">Foundation</span>','<span class="release-tag tag-major" data-i18n="changelogTagFoundation">Foundation</span>','changelogTagFoundation');

fs.writeFileSync(FILE, html, 'utf8');
console.log('changelog.html done — ' + count + ' replacements.');
